...
let ultimoTituloStreaming = '';
let ultimoArtistaStreaming = '';
...
btnRwd.addEventListener('click', () => {
  ...
  cargarTrack(currentTrack);
  console.log(`[ALEXIA] Retroceso a pista ${currentTrack}`);
});
...
btnFwd.addEventListener('click', () => {
  ...
  cargarTrack(currentTrack);
  console.log(`[ALEXIA] Avance a pista ${currentTrack}`);
});
...
function activarStreaming() {
  ...
  audio.load();
  actualizarMetadatosStreaming(emisora);
...
// 🔁 Lógica Principal: actualizar metadatos solo si hay cambios
async function actualizarMetadatosStreaming(nombreBusqueda) {
  ...
  const tituloNuevo = resultado.titulo?.trim() || '';
  const artistaNuevo = resultado.artista?.trim() || '';

  if (
    tituloNuevo !== ultimoTituloStreaming ||
    artistaNuevo !== ultimoArtistaStreaming
  ) {
    aplicarMetadatos(resultado);
    ultimoTituloStreaming = tituloNuevo;
    ultimoArtistaStreaming = artistaNuevo;
    console.log(`[ALEXIA] Metadatos actualizados: ${tituloNuevo} — ${artistaNuevo}`);
  } else {
    console.log('[ALEXIA] Sin cambios en metadatos de streaming');
  }
}